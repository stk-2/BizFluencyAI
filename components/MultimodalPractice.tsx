...
interface MultimodalPracticeProps {
  model: string;
  apiKey: string;
}

const MultimodalPractice: React.FC<MultimodalPracticeProps> = ({ model, apiKey }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const res = await describeBusinessImage(image, finalPrompt, model, apiKey);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Visual Description Practice</h2>
        <p className="text-slate-500 dark:text-slate-400">Upload a chart, graph, or slide and practice explaining it in English.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-colors">
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-8 bg-slate-50 dark:bg-slate-900/50">
          {image ? (
            ...
            <div className="text-center cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              ...
              <p className="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Click to upload image</p>
        ...
        <div className="mt-6">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Specific Question (Optional)</label>
            <input 
                type="text" 
                value={prompt}
      ...
      {response && (
        <div className="bg-orange-50 dark:bg-orange-900/30 border border-orange-100 dark:border-orange-800 p-6 rounded-xl">
            <h3 className="font-bold text-orange-900 dark:text-orange-300 mb-2">Model Description:</h3>
            <p className="whitespace-pre-wrap text-slate-800 dark:text-slate-200 leading-relaxed">{response}</p>
        </div>
      )}
    </div>
  );
