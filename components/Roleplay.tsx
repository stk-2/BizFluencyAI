...
const Roleplay: React.FC<RoleplayProps> = ({ user }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const feedbackJson = await getFeedbackOnMessage(userMsg.text, selectedScenario.title);
      let feedback = null;
      try {
  ...
  if (!selectedScenario) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <h2 className="text-2xl font-bold text-slate-800 mb-6">Select a Roleplay Scenario</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SCENARIOS.map(scenario => (
            <button
              key={scenario.id}
              onClick={() => handleStart(scenario)}
              className="flex flex-col items-center text-center p-6 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-indigo-300 transition-all"
            >
    ...
    <div className="max-w-4xl mx-auto h-[calc(100vh-140px)] flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
        <div className="flex items-center gap-2">
           <button onClick={() => setSelectedScenario(null)} className="text-slate-400 hover:text-slate-600 font-bold mr-2">&larr;</button>
           <div>
             <h3 className="font-bold text-slate-800">{selectedScenario.title}</h3>
             <p className="text-xs text-slate-500">Role: {selectedScenario.systemInstruction.split('.')[0]}</p>
           </div>
        </div>
        <div className="px-3 py-1 bg-indigo-100 text-indigo-700 text-xs rounded-full font-medium">
          Target: {user.level}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50" ref={scrollRef}>
        {messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            {/* Feedback Bubble */}
            {msg.feedback && (
              <div className="mt-2 max-w-[75%] bg-amber-50 border border-amber-100 p-3 rounded-lg text-xs text-amber-800 flex gap-2">
      ...
      <div className="p-4 bg-white border-t">
        ...
        >
          <input
            type="text"
            className="flex-1 px-4 py-3 border border-slate-300 rounded-full focus:ring-2 focus:ring-indigo-500 outline-none"
